/**
 * Live Paper Trading Bot — Walk-Forward Validated Config
 *
 * Symbol universe and strategy params validated 2026-04-28 (v2 update):
 *   - 158 trades | Win rate 45.57% | PF 1.82 | Sharpe 3.76 | Expectancy +$15.41/trade
 *   - Walk-forward promotions: ORCL, MU, LRCX, Open window, Trend regime, Breakout setup
 *   - All 7 metric targets PASS
 *
 * Changes from v1 (8-symbol, $2,231):
 *   - Added ON  (+$153, PF 2.97 — 10 trades, WF eligible)
 *   - Added LRCX (+$89, PF 1.43 — 12 trades, WF promote)
 *   - Removed AMAT (WF watch, poor test-period, dragged WR below 45%)
 *   - Tightened stagnation: 25→20 min (cut stuck losers faster)
 *   - Tightened early-reversal: 12min/0.30 → 10min/0.25
 *   - Net improvement: +$203 P&L, +0.25 Sharpe, +0.07 PF
 *
 * Key differences from live-paper-trade.mjs:
 *   - Validated 9-symbol universe (ORCL, INTC, MU, NET, TSM, TSLA, NVDA, ON, LRCX)
 *   - 5Min bars only (1Min not validated)
 *   - Tightened strategy params (1.0% trailing stop, 20min stagnation, etc.)
 *   - Breakout-setup-only filter
 *   - Behavior profile whitelist (confirmationRequired, neutral, extendedBreakout)
 *
 * Usage:
 *   node scripts/live-paper-trade-validated.mjs
 *   BOT_MAX_NOTIONAL=500 node scripts/live-paper-trade-validated.mjs
 *
...
// Walk-forward validated universe — v2 (2026-04-28).
// Promoted: ORCL (PF 3.20, WF promote), MU (PF 1.96, WF promote), LRCX (PF 1.43, WF promote).
// Eligible: INTC, NET, TSM, TSLA, NVDA, ON (all net positive; small WF samples on ON/NET/NVDA).
// Removed: AMAT (WF watch, test-PF 0.96, dragged WR below 45% target).
const VALIDATED_SYMBOLS = ['ORCL', 'INTC', 'MU', 'NET', 'TSM', 'TSLA', 'NVDA', 'ON', 'LRCX']
let SYMBOLS = String(process.env.BOT_SYMBOLS || VALIDATED_SYMBOLS.join(','))
...
// confirmationRequired: 44.3% WR, +$1947 in backtest — the dominant profitable profile (v2).
// neutral: 60% WR, +$343. extendedBreakout: 75% WR, +$145.
...
function printDashboard() {
  ...
  console.log(`\n🤖  VALIDATED PAPER BOT  |  ${TIMEFRAME} bars  |  ${etTime} ET  |  PAPER 💵`)
...
async function main() {
  console.log(`\n🚀  VALIDATED PAPER BOT — 5Min Breakout Strategy`)
  console.log(`   Universe  : ${SYMBOLS.join(', ')}`)
