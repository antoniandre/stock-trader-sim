...
.glass-box.pa6
  .w-flex.justify-space-between.align-center.mb4
    h2.title2 Open Positions
    w-button(@click="refreshPositions" text round)
      w-icon.mr2(:spin="loading" sm) wi-spinner
      | Refresh

  .positions-list(v-if="positions.length > 0")
    .position-item(v-for="position in positions" :key="position.symbol")
      .w-flex.justify-space-between.align-center.py3.px4.border-bottom
        .position-info
  ...
  .loading-state(v-if="loading && !positions.length")
    .text-center.py8
      w-icon.mb4(spin size="32") wi-spinner
      .title3 Loading Positions...
</template>
